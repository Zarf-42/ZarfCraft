...
#func add_voxel(pos: Vector3i, voxel: TerrainAlgorithm.Voxel):
...
func generate_chunks(pos):
	...
	var chunk_avg: int = 0
	var chunk_num: int = 0
	var start_chunk_total_time = Time.get_ticks_msec()
	...
	for chunk in pos:
		...
		var stop_time = Time.get_ticks_msec()
		var chunk_gen_time = stop_time - start_time
		
		if chunk_gen_time > 2:
			chunk_avg = chunk_avg + chunk_gen_time
			chunk_num += 1
			#print(chunk_gen_time)
	if chunk_avg != 0 && chunk_num != 0:
		chunk_avg = chunk_avg / chunk_num
	var stop_chunk_total_time = Time.get_ticks_msec()
	var chunk_time: float = (stop_chunk_total_time - start_chunk_total_time)
	chunk_time = chunk_time/ 1000
	print("Generated %s chunks, each took ~%s." %[chunk_num, chunk_avg])
	print("It took %f seconds to generate all chunks on this thread." % [chunk_time])
